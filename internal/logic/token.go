...
func pemEncodePublicKey(publicKey *rsa.PublicKey) ([]byte, error) {
	...
	// encode public key base on algo defined in header
...
func NewToken(
	...
	if err != nil {
		logger.With(zap.Error(err)).Error("fail to create rsa key pair")
		return nil, err
	}
	// encode key to bytes
	publicKeyBytes, err := pemEncodePublicKey(&rsaKeyPair.PublicKey)
	if err != nil {
		logger.With(zap.Error(err)).Error("fail to encode public key to bytes")
		return nil, err
	}
	//
